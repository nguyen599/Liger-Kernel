...
def cross_entropy_forward(
    ...
    torch._assert(isinstance(return_token_accuracy, bool),
    ...
    torch._assert((target * target_mask).max() < _input.shape[-1], (
        f"Target {target.max()} is out of bounds. Expected < {_input.shape[-1]}"
    ))
    torch._assert((target * target_mask).min() >= 0, f"Target {target.min()} is out of bounds. Expected >= 0")
    sum_non_ignore_weight = n_non_ignore
    weight_sum = 0.0
    if weight is not None:
        torch._assert(weight.shape[0] == V, f"If given, weight has to be a Tensor of size V. Got: {weight.shape}")
        torch._assert(torch.is_floating_point(weight), (
